...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0 text-gray-800">Edit Service Category</h2>
            ...
            <form action="{{ route('admin.service-categories.update', $serviceCategory->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        <span class="input-group-text" id="icon-preview"><i class="{{ $serviceCategory->icon ?? 'fas fa-sitemap' }}"></i></span>
                        <input type="text" class="form-control" id="icon" name="icon" value="{{ old('icon', $serviceCategory->icon) }}" placeholder="e.g. fas fa-sitemap">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="status" name="status">
                        <option value="1" {{ (old('status') ?? $serviceCategory->status) == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ (old('status') ?? $serviceCategory->status) == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
